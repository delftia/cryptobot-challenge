...
async function main() {
    ...
    const users = [];
    await Promise.all(Array.from({ length: cfg.users }).map((_, i) => limit(async () => {
    ...
    async function tick() {
        ...
        const batch = Array.from({ length: cfg.concurrency }).map(() => limit(async () => {
        ...
        if (snipeMode) {
            ...
            await Promise.all(Array.from({ length: cfg.concurrency }).map(() => limit(async () => {
                const u = users[Math.floor(Math.random() * users.length)];
                const prev = bids.get(u.id) ?? 0;
        ...
        if ((Date.now() - start) % 5000 < 200) {
            ...
            console.log(`[t+${Math.floor((Date.now() - start) / 1000)}s] round=${a.auction.currentRound} remaining=${a.auction.remainingItems} endsAt=${a.auction.currentRoundEndsAt} ext=${a.auction.currentRoundExtendedBySec}s inv_ok=${inv.ok} ops=${ops} err=${errors}`);
            if (!inv.ok) {
